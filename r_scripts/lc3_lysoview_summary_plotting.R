# lc3_lysoview_summary_plotting.R
...
# load data
input_df <- read_csv('~/Dropbox/code/csth-imaging/output_files/lc3_lysoview_summary_analysis_output.csv')
...
ggplot(subset(for_plt, cell_line != 'dVPS37A'), aes(x=factor(treatment), y=count)) +
  ...
  labs(x='Torin treatment time (hr)',
       ...
       title='Cell line')
ggsave('~/Dropbox/code/csth-imaging/r_scripts/lc3_lysoview_plots/lc3_lysoview_totals_boxplot.pdf',
...
ggplot(subset(for_plt, cell_line != 'dVPS37A'), aes(x=treatment, y=overlap_count)) +
  ...
  labs(x='Torin treatment time (hr)',
       ...
       title='Cell line')
ggsave('~/Dropbox/code/csth-imaging/r_scripts/lc3_lysoview_plots/lc3_lysoview_overlap_boxplot.pdf',
  device=cairo_pdf(width=7.5, height=4), useDingbats=FALSE)
